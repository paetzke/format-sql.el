...
Provides the =py-format-sql-buffer= command, which uses the external [[https://github.com/paetzke/format-sql][format-sql]] tool to format SQL in the current buffer.
